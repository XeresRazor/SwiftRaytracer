...
private func perlinGenerate() -> [double4] {
	var p = [double4](count: 256, repeatedValue: double4(0,0,0,1))
	for i in 0 ..< 256 {
...
private func permute(inout p: [Int]) {
	var i = p.count - 1
	while i > 0 {
...
private func perlinGeneratePermutation() -> [Int] {
	var p = [Int](count: 256, repeatedValue: 0)
	for i in 0 ..< 256 {
		p[i] = i
	}
	permute(&p)
	...
	for i in 0 ..< 2 {
		for j in 0 ..< 2 {
			for k in 0 ..< 2 {
				...
				accum += (Double(i) * uu + Double(1 - i) * Double(1 - uu)) *
					(Double(j) * vv + Double(1 - j) * Double(1 - vv)) *
					(Double(k) * ww + Double(1 - k) * Double(1 - ww)) *
					dot(c[i][j][k], weightV)
			}
		}
	}
...
public struct Perlin {
	...
	public func noise(point: double4) -> Double {
		...
		var c = [[[double4]]](count: 2, repeatedValue: [[double4]](count: 2, repeatedValue: [double4](count: 2, repeatedValue: double4(0,0,0,0))))
		...
		return perlinInterpolation(c, u: u, v: v, w: w)
	}
}
