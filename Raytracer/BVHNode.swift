...
public class BVHNode: Traceable {
	...
	public init(list l: [Traceable], time0: Double, time1: Double) {
		...
		if axis == 0 {
			list.sortInPlace({ (ah, bh) -> Bool in
				let leftBox = ah.boundingBox(0, t1: 0)
				let rightBox = bh.boundingBox(0, t1: 0)
		...
		let boxLeft = left.boundingBox(time0, t1: time1)
		let boxRight = right.boundingBox(time0, t1: time1)
